...
export function OwnPerformanceOverview({ 
  goals, 
  performanceScore, 
  gradeInfo,
  evaluationStatus 
}: OwnPerformanceOverviewProps) {
  const { t } = useTranslation();

  const getCurrentStatus = () => {
    if (!goals.length) return 'awaiting_goal_setting';
    
    // Get the most common status
    const statusCounts = goals.reduce((acc, goal) => {
      const status = goal.evaluation_status || 'awaiting_goal_setting';
            ...
            <div className="text-gray-700 dark:text-gray-300">
              {evaluationStatus ? (
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                  {t(`admin.evaluation.statuses.${evaluationStatus}`)}
                </span>
              ) : (
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                  {t(`admin.evaluation.statuses.${getCurrentStatus()}`)}
                </span>
              )}
            </div>
          </div>
