...
export function PerformanceScore({ goals }: PerformanceScoreProps) {
  ...
  const fetchGrades = async () => {
    try {
      ...
      setGrades([
        { id: '1', organization_id: null, min_score: 0, max_score: 20, grade_text: t('goals.evaluation.performance.grades.unsatisfactory'), grade_level: 1, created_at: '', updated_at: '' },
        { id: '2', organization_id: null, min_score: 21, max_score: 40, grade_text: t('goals.evaluation.performance.grades.weak'), grade_level: 2, created_at: '', updated_at: '' },
        { id: '3', organization_id: null, min_score: 41, max_score: 60, grade_text: t('goals.evaluation.performance.grades.normal'), grade_level: 3, created_at: '', updated_at: '' },
        { id: '4', organization_id: null, min_score: 61, max_score: 80, grade_text: t('goals.evaluation.performance.grades.good'), grade_level: 4, created_at: '', updated_at: '' },
        { id: '5', organization_id: null, min_score: 81, max_score: 100, grade_text: t('goals.evaluation.performance.grades.excellent'), grade_level: 5, created_at: '', updated_at: '' },
      ]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className={`text-lg font-medium ${getScoreColor(performanceScore)}`}> 
              {t(`goals.evaluation.performance.grades.${gradeInfo.grade_text.toLowerCase()}`)}
            </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-600">
                {grades.map((grade) => (
                  <tr key={grade.id} className={performanceScore >= grade.min_score && performanceScore <= grade.max_score ? 'bg-indigo-50 dark:bg-indigo-900/50' : ''}>
                    ...
                    <td className="px-4 py-2 text-sm text-gray-900 dark:text-gray-100">
                      {t(`goals.evaluation.performance.grades.${grade.grade_text.toLowerCase()}`)}
                    </td>
                    <td className="px-4 py-2 text-sm text-gray-900 dark:text-gray-100">
                      {grade.grade_level}
                    </td>
